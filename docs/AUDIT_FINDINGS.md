...
18. **Analysis 2, IMERG 11-year vs. 3-year threshold.** *Decided 2026-09-22 (GSMaP section above).* Kept the 3-year threshold for production; the interannual IQR is measurably larger at 11 years (~1.36-1.38x domain-mean) but the P90/P95 maps themselves change only ~1-2%, and the storm-type attribution was not re-run. The 11-year IMERG and GSMaP threshold files are kept as archival references (`_2014_2024` suffix), outside the pipeline registry, for a future revisit of this decision. **Revised 2026-09-23 (user).** After reviewing the two key manuscript figures made with the 2014-2024 thresholds (`plot_cof_extreme_raintype_rank_map.ipynb`), the user judged the difference for IMERG negligible and the interannual standard deviations of the type contributions small (measured: the type shares of the 60°S-60°N extreme precipitation change by at most 0.25 percentage points, the regional means of the pie charts by up to 4 points, tropical-ocean MCS), and the production IMERG attribution files (`IMERGv7_stormtype_spatial_p90.nc`, `_p95.nc`, pscratch and CFS) were regenerated with the 11-year threshold file through the new `--threshold_file` option. They also carry the new per-year variables. The masks and the analyzed record (2019-2021) are unchanged, and the model sources' files are unchanged. The previous files (2019-2021 thresholds, whole-record variables only) are archived in `extreme_precip/_prev_production_IMERGv7_thresholds_2019_2021_20260920/` on both filesystems. `IMERGv7_precip_percentiles_6h_hp8_v1.nc` itself is still the 3-year file (the threshold-map notebooks read it) but the runner no longer makes it: the IMERG entry of `config/config_pipeline.yaml` now computes the thresholds from the 11-year store (`thresholds: version: v1_2014_2024`, output `IMERGv7_precip_percentiles_6h_hp8_v1_2014_2024.nc`) and its attribution reads that file, so `run_cof_pipeline.py` reproduces the production files (checked 2026-09-23 with a runner run of both steps in a scratch data root: the thresholds file is byte-identical to the existing 11-year one, the attribution files to production apart from `created_on` and the path in `threshold_file`). The attribution script's own default for IMERG is the same file (`threshold_version: v1_2014_2024` in `config_sources.yaml`), so a direct call and the older wrappers (`run_stormtype_extreme_precip.sh`, the slurm scripts) also give the production version. **What the 11-year thresholds change** (measured 2026-09-23): the 11-year 6-hourly store is not the 3-year store extended. Over 2019-2021 the two fields differ (60 evenly spaced shared steps: correlation 0.996, mean absolute difference 0.011 mm/h at equal means, no time shift), so the shift of the thresholds has two parts. For the cells within 60S-60N the median P95 goes from 2.629 to 2.734 mm/h (+4.0%; P90 1.745 to 1.790, +2.6%): +1.2% from the store (thresholds of the 11-year store over 2019-2021: 2.660 mm/h; individual cells -5% to +8%) and +2.8% from the longer record (individual cells -17% to +44%, 5th-95th percentile).
19. **`plot_extreme_rain_threshold_map.ipynb`'s `shade_interannual_iqr` toggle.** *Off by default (GSMaP section above), needs a decision only if revisited.* Production's IMERG threshold file has no annual/IQR variables, so the toggle is off and the zonal panel shows plain lines for every source. Flipping it on needs a source in `rootdir` whose file has `pr_annual_p*`/`year` (GSMaP already qualifies; IMERG would need to be swapped for the 3- or 11-year version this round regenerated, which was deliberately not done to production). **Revised 2026-09-24 (user).** `plot_extreme_rain_threshold_map.ipynb` now reads the 11-year (2014-2024) IMERG and GSMaP files (`file_version` in its config cell; the five GSRMs keep their own one-year files) and `shade_interannual_iqr` is `True`: the zonal-mean panel shades, for IMERG and GSMaP only, the interquartile range across the 11 years of the zonal-mean annual percentile; the GSRMs have no per-year variables and stay unshaded. The plotted line is the pooled 2014-2024 percentile, a few percent above the mean of the yearly values, so at P95 it lies above the band in most latitude bins (IMERG 44 of 60 bins within 60S-60N, GSMaP 36; 17 and 5 at P90), whereas the companion notebook `plot_extreme_rain_threshold_map_obs_interannual.ipynb` draws the interannual mean, which lies inside its band in 91-100 % of the bins. GSMaP has few valid cells poleward of 80N (about half in the pooled file, 9-28 % per year) and almost none poleward of 80S, so its polar zonal means and band are noisy (a spike near 85-88N; no band in the two southernmost bins). The zonal panel's x axis is 90S-90N (ticks every 30 degrees), the band alpha is 0.4, and the band is computed by `zonal_iqr` in both notebooks, which avoids numpy's 'All-NaN slice encountered' warning.
