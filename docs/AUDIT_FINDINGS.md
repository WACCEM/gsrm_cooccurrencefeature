...
- **The radii of the spatial statistics are in grid points, not degrees.** `x` and `y` of the combined store are offsets in grid points (-80 to 80, 0.25° apart), and `create_circular_mask` compares them with the radius as given. `--basic-radius 10`, `--pr-radius 10` and the mask radii `10` and `15` (`*_frac_r10`, `*_frac_r15`,
  the attributes `basic_radius_deg`, `pr_radius_deg`, `mask_radii_deg`, the notebook's printed "10° radius") are therefore 2.5° and 3.75° (checked: `pr_mean` of the statistics equals the mean of `pr` inside a circle of 10 grid points, correlation 1.0000, not inside 40). This holds for the March and for the new statistics alike, so the comparisons are not affected;
  correcting it (radii 40 and 60 grid points, or new labels) changes every statistic and needs a decision.
- **The "all" composites contain the points that have no frame of the source.** Their `pr` is NaN and skipped by the mean, but the mask frequencies (`xr.where(mask > 0, 1, 0)` turns NaN into 0) and the exclusive precipitation (`pr_ar`, `pr_mcs`, `pr_etc`, 0 where the mask is NaN) count them as zeros, so `*_freq` and `pr_ar/pr_mcs/pr_etc` of the `all` files
  are low by the share of those points in the hemisphere (points with NaN `overlap_flag`: ICON 17.3% NH / 13.4% SH, UM 4.5% / 3.8%, SCREAM 1.4% / 1.7%, ERA5 0.9% / 1.1%, NICAM 0.6% / 1.1%, CASESM2 0.6% / 1.0%; almost all of ICON's, UM's and SCREAM's are the points without a frame). ICON had the same in March (masks NaN). The files with a flag (`isolated`, `mcs_only`,
  `ar_only`, `3way`) and the statistics filtered by `overlap_flag` are not affected. A fix is one line (keep NaN where the mask is NaN, or leave the points with NaN flag out of `all`); it changes the `all` files only.
...
9. **Analysis 3: radii of the spatial statistics.** Not changed, needs a decision: the statistics' radii (10 and 15) are grid points of 0.25° (2.5° and 3.75°), not degrees (Analysis 3 section, "Not changed").
10. **Analysis 3: the `all` composites include points without a frame.** Not changed, needs a decision: their mask frequencies and exclusive precipitation are low by the share of those points (ICON 17% NH, 13% SH; UM about 4%; SCREAM 1.4-1.7%);
    a one-line change to the composite step fixes it (Analysis 3 section, "Not changed").
11. **Analysis 3 outputs are in a test area, not in production.** The core-tier run wrote to `/pscratch/sd/w/wcmca1/hackathon/tmp/etc_round1/` (`etc_tracks/`, `etc_data/<source>/`, `etc_data/stats/`, the figures of the notebook comparison and the run markers and logs). Production `etc_data/`
    (the March combined stores, composites and statistics, and the environment stores that were linked) was only read and is unchanged. Promotion is the user's decision; the runner protects the production outputs (they have no markers, so a run with `--data-root` set to the production tree stops unless `--force`).
