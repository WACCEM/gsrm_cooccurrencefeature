...
17. **Globus copy of the Analysis 3 and 4 items.** *Pending (user).* The 145 new items (263.0 GB) are listed in `NEW_ITEMS_for_globus_A3A4.txt` in the pscratch archive folder; the CFS counterparts were moved into the CFS archive folder first, so a copy of the folders `etc_data`, `etc_tracks` and `cof_masks` (sync level 2) mirrors production. A file-by-file comparison of pscratch and CFS for these three folders (a file is copied when it is missing at the destination or newer at the source) finds exactly the 145 new items to copy (8,490 files, 263.0 GB, every file of every item) and nothing else; 7,761 files exist only on CFS and are untouched when "delete files on destination" is off.
