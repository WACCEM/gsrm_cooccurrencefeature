...
drop out of the composites of a single flag and out of the statistics that are filtered by `overlap_flag`. They are still in the `all` composites (see "Known limits" below) and in the environment variables, which do not depend on the COF period. Each extraction records `n_points`, `n_points_time_missing` and `n_points_failed` in the store attributes and fails before writing when more points are
...
## Known limits of the composites and statistics (not changed by the runner)

- **The radii of the statistics are in grid points.** `x` and `y` are offsets in grid points (0.25° apart); `--basic-radius 10`, `--pr-radius 10` and the mask radii 10 and 15 (`*_frac_r10`, `*_frac_r15`) are 2.5° and 3.75°, although the attributes and the notebook say degrees.
- **The `all` composites include the points that have no frame of the source** (ICON 17% of the northern and 13% of the southern points, UM about 4%, SCREAM 1.4-1.7%): their `pr` is NaN and skipped, but the mask frequencies and the exclusive precipitation (`pr_ar`, `pr_mcs`, `pr_etc`) count them as zeros, so these variables of the `all` files are low by that share.
  The files of a single flag and the statistics filtered by `overlap_flag` are not affected.
