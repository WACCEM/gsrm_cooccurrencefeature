#!/bin/bash
# Run Analysis 3 (the ETC composites) for any set of sources on an interactive node (no queue wait, 4 h limit).
# It runs scripts/run_etc_pipeline.py in the background with nohup, so a lost connection does not stop it.
#
# Usage:
...
#   2. Run this script with the options of run_etc_pipeline.py; --data-root is required (a test area first: the new products are
#      written there, the COF products in cof_masks/ and the environment stores in etc_data/ are only read):
#        bash slurm/run_interactive_etc_pipeline.sh --data-root /pscratch/sd/w/wcmca1/hackathon/tmp/etc_round1 --dry-run
#        bash slurm/run_interactive_etc_pipeline.sh --data-root /pscratch/sd/w/wcmca1/hackathon/tmp/etc_round1
#        bash slurm/run_interactive_etc_pipeline.sh --data-root DIR --sources um scream
#        bash slurm/run_interactive_etc_pipeline.sh --data-root DIR --resume        # continue after a stop or a timeout
...
#   python scripts/run_etc_pipeline.py --help      and      docs/procedures/run_etc_pipeline.md

REPO="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
PYTHON="${COF_PYTHON:-/global/common/software/m1867/python/hackathon/bin/python}"
RUNNER="${REPO}/scripts/run_etc_pipeline.py"
...
if [ -z "$DATA_ROOT" ]; then
    echo "--data-root is required (a test area first)"; exit 2
fi
...
if ! "$PYTHON" "$RUNNER" "${ARGS[@]}" --preflight-only > /tmp/etc_preflight_$$.txt 2>&1; then
    echo "The input checks failed:"; grep -i "problem\|failed" /tmp/etc_preflight_$$.txt | head -10
    rm -f /tmp/etc_preflight_$$.txt
    exit 2
fi
rm -f /tmp/etc_preflight_$$.txt
