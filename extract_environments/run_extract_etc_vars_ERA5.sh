...
# ===== INTERACTIVE RUN SCRIPT FOR ERA5 =====
# This script is for running ETC 2D extraction for ERA5 data in an interactive terminal session
# ERA5 tracks are on a structured lat/lon grid (not unstructured HEALPix mesh)
# 
# Usage:
...
#   3. Run this script:
#      bash run_extract_etc_vars_ERA5.sh
#
...
ROOT_DIR="/pscratch/sd/w/wcmca1/hackathon/etc_tracks"  # UPDATE THIS PATH
TRACK_FILE="${ROOT_DIR}/era5.etc_stitched_nodes.txt"  # UPDATE THIS PATH
OUTPUT_DIR="/pscratch/sd/w/wcmca1/hackathon/etc_data/tests/era5/single_vars/"  # UPDATE THIS PATH
...
CURRENT_LOCATION=""  # Empty string = None (catalog opened directly without location)
CATALOG_MODEL="era5_3h"  # ERA5 model
...
# Build base command
CMD="python extract_etc_2d_vars.py \
